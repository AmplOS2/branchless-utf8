...
static long randchar(uint64_t *s) {
        uint32_t r   = pcg32(s);
        int      len = 1 + (r & 0x3);
        r >>= 2;
        switch(len) {
 ...
 * lengths.
 */
static uint8_t *buffer_fill(uint8_t *buf, size_t z) {
        uint64_t s   = 0;
        uint8_t *p   = buf;
        uint8_t *end = p + z;
        while(p < end) {
...
static unsigned char *utf8_simple(unsigned char *s, long *c) {
        unsigned char *next;
        if(s[0] < 0x80) {
                *c   = s[0];
                next = s + 1;
...
int main(void) {
        ...
        uint8_t *buffer = (uint8_t *)malloc(z);
        uint8_t *end    = buffer_fill(buffer, z - 4);
        ...
        do {
                ...
                long           count = 0;
                for(; p < end; p++) {
                        if(!bh_utf8_decode(&state, &c, *p)) count++;
